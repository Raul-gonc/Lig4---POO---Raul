...
public class GameTurboMalucoUI {
	...
	/*public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					GameTurboMalucoUI window = new GameTurboMalucoUI();
	 ...
	 */
	public GameTurboMalucoUI() {
		initialize(new Player("Sem Nome 1","y"), new Player("Sem Nome 2","r"), 3);
		frame.setVisible(true);
	}
	
	public GameTurboMalucoUI(Player player1, Player player2, double dif) {
		initialize(player1, player2, dif);
		frame.setVisible(true);
	}
	...
	GameTurboMaluco game;
	Ranking rank = new Ranking();
	
	private void endGame() {
		if(game.getBoard().verifyWin()) {
			rank.adicionarJogador(game.getPlayer().getName());
			System.out.println(game.getPlayer().getName()+"WIN");
			frame.dispose();
			
			String mensagem = "O jogador " + game.getPlayer().getName() + " venceu!";
	        JOptionPane.showMessageDialog(null, mensagem, "Vitória", JOptionPane.INFORMATION_MESSAGE);
			
			MenuPrincipal window = new MenuPrincipal();
		}else if(game.getBoard().verifyDraw()) {
	...
	private void gameStart(Player player1, Player player2, double dif) {
		game = new GameTurboMaluco(player1, player2, dif);
	}
	
	private void initialize(Player player1, Player player2, double dif) {
		frame = new JFrame();
		gameStart(player1, player2, dif);
